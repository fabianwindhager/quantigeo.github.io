...
p_mig <- ggplot(mig_typ %>% filter(variable == "mig_rate"), aes(x = as.factor(jahr), y = value, group = typ, color = typ))+
  geom_line(linewidth = 1)+
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
    ...
    labs(y = "Rate pro 1000 EW",
       x = NULL,
       color = NULL,
       title = "Gesamtmigrationsrate; Tirol 2003-2023") 

p_bi <- ggplot(mig_typ %>% filter(variable == "bi_rate"), aes(x = as.factor(jahr), y = value, group = typ, color = typ))+
  geom_line(linewidth = 1)+
  geom_hline(yintercept = 0, col = "black", linetype = "dashed", size = 0.7)+
  scale_color_manual(
    values = c("Urbaner Raum" = "#b35e6b",
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
    ...
    labs(y = "Rate pro 1000 EW",
       x = NULL,
       color = NULL,
       title = "Binnenmigrationsrate; Tirol 2003-2023") 

p_au <- ggplot(mig_typ %>% filter(variable == "au_rate"), aes(x = as.factor(jahr), y = value, group = typ, color = typ))+
  geom_line(linewidth = 1)+
  geom_hline(yintercept = 0, col = "black", linetype = "dashed", size = 0.7)+
  scale_color_manual(
    values = c("Urbaner Raum" = "#b35e6b",
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
    ...
    labs(y = "Rate pro 1000 EW",
       x = NULL,
       color = NULL,
       title = "Aussenwanderungsrate; Tirol 2003-2023") 

p_nat <- ggplot(mig_typ %>% filter(variable == "nat_rate"), aes(x = as.factor(jahr), y = value, group = typ, color = typ))+
  geom_line(linewidth = 1)+
  geom_hline(yintercept = 0, col = "black", linetype = "dashed", size = 0.7)+
  scale_color_manual(
    values = c("Urbaner Raum" = "#b35e6b",
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
    ...
    labs(y = "Rate pro 1000 EW",
       x = NULL,
       color = NULL,
       title = "Natürliche Bevölkerungsbewegung; Tirol 2003-2023") 
```

```{r}
library(patchwork)

(p_mig + p_bi + p_au + p_nat) +
